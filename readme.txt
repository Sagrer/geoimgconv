...
с исходным текстом. Сборка программы тестировалась в Debian 9, Ubuntu 17.10,
а также под Windows в MinGW и в Visual Studio 2015, 2017.

	О том, как правильно собрать программу под linux - читайте файл
readme_linux.txt
	О сборке под windows - читать readme_windows.txt.
	
	Программа зависит от следующих библиотек: boost, gdal, а также как зависимости
этих библиотек - опционально iconv и/или icu.
...
ньюансы, о которых можно прочитать в вышеназванных файлах.
	Также для сборки применяется CMake и Cotire, при этом часть модулей CMake
и модуль Cotire включены в исходный код (см каталог source/cmake_modules).
 ...
 * Длина строк текста исходных файлов - автоматически не контролируется, но по
возможности нужно стараться не выходить за разумные рамки около 100 символов.
 ...
 * Имена констант, #define-ов, элементов enum-ов (но не enum-классов) записываются в
