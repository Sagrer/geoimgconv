@rem Скрипт для сборки BOOST под 2017 студию.
...
.\b2 --build-dir=build_vs2017 --build-type=complete stage --stagedir=build_vs2017\stage -sICU_PATH=%DevLibsVs2017% --abbreviate-paths toolset=msvc-14.1 boost.locale.icu=on threading=multi architecture=x86 address-model=32
@echo.
if NOT %ERRORLEVEL% == 0 (
  echo FAILED!!!
  pause
