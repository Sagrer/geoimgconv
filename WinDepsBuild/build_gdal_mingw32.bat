@rem Скрипт для сборки GDAL под 32-битный mingw. Должен быть установлен msys2
@echo off
...
@rem префикс mingw
set mingw_prfx=/%msysdir:\=/%/mingw32
...
@rem Но помимо прочего - именно для 32-битной версии приходится навешивать ещё один патчик:
sed -i port/cpl_port.h -e "s|x86intrin.h|intrin.h|g"
if NOT %ERRORLEVEL% == 0 (
  @echo.
  echo FAILED!
  @echo.
